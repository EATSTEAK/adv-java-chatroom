...
TUI는 `Rooms`, `Chat`, `Users`, `Raw Log` pane으로 구성됩니다. `Tab`/`Shift-Tab` 또는 `h`/`l`, 방향키 좌우로 pane을 전환하고, `j`/`k` 또는 방향키 상하로 방과 유저를 선택합니다. 마우스 클릭은 pane과 선택 행만 바꾸며, 명령은 단축키로 보냅니다.

주요 단축키:

- 시작 시 `login>` 입력창에 사용자명을 입력하고 `Enter`를 누르면 로그인합니다. 미로그인 상태에서 로그인 입력은 `l`로 다시 열 수 있습니다.
- `Rooms` pane에서 `a`는 방 생성, `Enter`는 선택 방 입장, `q`는 선택 방 퇴장입니다.
- `Chat` pane에서 `Enter` 또는 일반 문자 입력은 현재 방 메시지 입력을 시작합니다.
- `Users` pane에서 `w` 또는 `Enter`는 선택 유저에게 귓속말 입력을 시작합니다.
- `r`은 방/유저 목록을 즉시 갱신하고, `:`는 raw 프로토콜 명령 입력을 엽니다.
- 입력 모드 중 `Esc`는 입력을 취소합니다. 평상시 `Esc`, `Ctrl-Q`, `Ctrl-C`는 종료 단축키이며, 연결이 살아 있으면 먼저 `QUIT`을 보냅니다.
